...
PROMPT_CONTEXT_DIR = BASE_DIR / "prompt_contextMixed"
...
def normalize_prompt_name(prompt_name: str) -> str:
    value = prompt_name.strip()
    if len(value) >= 2 and value[0] == value[-1] and value[0] in {"\"", "'"}:
        value = value[1:-1].strip()
    return value


def get_context(prompt_name: str) -> str:
    normalized = normalize_prompt_name(prompt_name)
    context_path = PROMPT_CONTEXT_DIR / f"{normalized}.txt"
...
def run_fextract(
    ...
    for attempt in range(2):
        messages = build_fextract_messages(
            question,
            student_answer,
            rubric,
            retry_note,
...
def grade_row(
    ...
    extracted, a1_pt, a1_ct, a1_errors, a1_error = run_fextract(
        question, answer, rubric, endpoint, api_key, deployment, api_version, context
    )
...
def main() -> int:
    ...
    for i, row in enumerate(rows_to_process, start=start_row):
        ...
        if not (row.get("prompt_name") or "").strip():
            raise SystemExit(f"Missing prompt_name in row {i}")
    ...
    for row in rows_to_process:
        ...
        context_path = PROMPT_CONTEXT_DIR / f"{normalized}.txt"
        if not context_path.exists():
            missing_contexts.append(str(context_path))
            continue
        context_cache[normalized] = load_text(context_path)

    if missing_contexts:
        print("Missing prompt_context files:")
        for path in missing_contexts:
            print(f"- {path}")
        raise SystemExit("Missing prompt_context files; fix prompt_name mapping before running.")
        ...
        with ThreadPoolExecutor(max_workers=500) as executor:
            while pending_rows or active_futures:
                while pending_rows and len(active_futures) < target_workers:
                    ...
                    prompt_name = (state["row"].get("prompt_name") or "").strip()
                    normalized_prompt = normalize_prompt_name(prompt_name)
                    future = executor.submit(
                        _grade_row_wrapper,
                        state["row_index"],
                        state["row"],
                        rubric_text,
                        context_cache[normalized_prompt],
                        endpoint,
                        api_key,
                        deployment,
                        api_version,
