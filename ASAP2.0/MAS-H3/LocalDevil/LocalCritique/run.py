...
# ──────────────────────────────────────────────────────────────────────────────
DEVILS_ADVOCATE_ALWAYS_SEND: bool = True
...
def run_fextract(
    ...
    deployment: str,
    api_version: str,
) -> tuple[dict[str, Any] | None, dict[str, Any] | None, int, int, int, int, int, int, str | None]:
    """
    Returns:
    ...
    for attempt in range(2):
        ...
        try:
            ...
            if attempt >= 1:
                return None, None, total_pt, total_ct, adv_pt, adv_ct, error_count, adv_error_count, \
                    f"fextract failed after retries: {exc}"
        ...
        if raw is None:
            return None, None, total_pt, total_ct, adv_pt, adv_ct, error_count, adv_error_count, None
        ...
        return None, None, total_pt, total_ct, adv_pt, adv_ct, error_count, adv_error_count, \
            f"fextract failed after retries: {last_error}"
    ...
    if not should_revise:
        return (
            ...
            error_count, adv_error_count, None,
        )
...
def run_fscoring(
    ...
    deployment: str,
    api_version: str,
) -> tuple[dict[str, Any], dict[str, Any] | None, int, int, int, int, int, int, str | None]:
    """
    Returns:
    ...
    for attempt in range(2):
        ...
        if raw is None:
            return (
                ...
                None, total_pt, total_ct, adv_pt, adv_ct, error_count, adv_error_count, None,
            )
...
def main() -> int:
    ...
    def adjust_workers(had_retryable_event: bool) -> None:
        ...
        if had_retryable_event:
            clean_success_streak = 0
            target_workers = max(80, target_workers - 1)
        else:
